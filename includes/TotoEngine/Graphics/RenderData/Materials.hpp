...
struct Material {
    Material(BasicMaterial& material)
        : data(material) {}
    Material(PositionMaterial& material)
        : data(material) {}
    Material(DepthMaterial& material)
        : data(material) {}
    Material(NormalMaterial& material)
        : data(material) {}
    Material(PhongMaterial& material)
        : data(material) {}

    std::variant<
        std::reference_wrapper<BasicMaterial>, std::reference_wrapper<PositionMaterial>,
        std::reference_wrapper<DepthMaterial>, std::reference_wrapper<NormalMaterial>,
        std::reference_wrapper<PhongMaterial>>
        data;

    void apply(ShaderProgram& shader) const {
        std::visit([&](auto&& material) { material.get().apply(shader); }, data);
    }
